PRO ENERGY_ASK, megablock, frame1, Nframes, background, energy, ratio, flux,int1_cal,int3_cal, save=save, wrap=wrap
...
read_vs, filename = [megablock+'r1.txt',megablock+'r3.txt'],/quiet
common vs

;get the intensity calibration
mjs0 = time_v(frame1,/full)   
cal_ask1 = get_ask_cal(mjs0,1)   ; =2.25
cal_ask3 = get_ask_cal(mjs0,3)   ; =1.70
...
print, 'Means w/ bg: ', mean(int1), mean(int3)
;Removing background
...
print, 'Means w/o bg: ', mean(int1), mean(int3)

;Applying calibration
int1_cal = (int1*cal_ask1)/vres(vsel) ;THIS IS WHAT WAS CAUSING DIFFERENCE IN FLUXES - CHANGED TO MULTIPLY BY VRES RATHER THAN DIVIDE 05/04/2018
int3_cal = (int3*cal_ask3)/vres(vsel)

if keyword_set(wrap) then begin
	print, 'WRAP --------------------------------'
	int3_cal = SHIFT(int3_cal, 1)
	;int3_cal[-5:-1] = int1_cal[-5:-1]*2
endif
